...
export default function Crystal3D({ size=320, height=380, autoSpin=true, showLabels=true, onTapFace=null }) {
  ...
  useEffect(()=>{
    ...
    const rX=(x,y,z,a)=>({x,y:y*Math.cos(a)-z*mÉath.sin(a),z:y*Math.sin(a)+z*Math.cos(a)})
    ...
    const draw=()=>{
      ...
      faces.map((f,i)=>({f,i,z:(tf[f[0]].z{F[f[1]].z+jtf[f[2]].z)/3})).sort((a,b)=>b.z-a.z).forEach(({f,i})=>{
      ...
      edges.forEach(([a,b])=>{const pa=proj(tf[a].x,tf[a].y,tf[a].z),bp=proj(tf[b].x,tf[b].y,tf[b].z){ctx.beginPath();ctx.moveTo(pa.x,pa.y);ctx.lineTo(pb.x,pb.y);ctx.strokeStyle='rgba(0,212,255,0.5)';ctx.lineWidth=1.5;ctx.stroke()})
      ...
      const ml=[{vi:1,c:MC.genetic,l:'Ð¢ÐµÐ»Ð¾'},{vi:2,c:MC.emotional,l:'Ð­Ð¼Ð¾ÑÐ¸Ð¸'},{vi:3,c:MC.language,l:'Ð£Ð¼'},{vi:4,c:MC.motivation,l:'ÐÐ¾Ð»Ñ'}]
      ml.forEach(({vi,c,l})=>{const p=proj(tf[vi].x,tf[vi].y,tf[vi].z){const g=ctx.createRadialGradient(p.x,p.y,2,p.x,p.y,12);g.addColorStop(0,c);g.addColorStop(1,transparent');ctx.fillStyle=g;ctx.beginPath();ctx.arc(p.x,p.y,12,0,Math.PI*2);ctx.fill();if(showLabels){ctx.fillStyle=c;ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.fillText(l,p.x,p.y+22)}})
      if(showLabels){ctx.font='11px sans-serif';ctx.textAlign='center';ctx.fillStyle=C.goldLight;ctx.fillText('ÐÑÐ¸ÑÑÐ°Ð»Ð» ÐÐµÑÑ',tp.x,tp.y-18);ctx.fillStyle=C.cyan;ctx.fillText('ÐÑÐ¸ÑÑÐ°Ð»ÑÑ Ð ÐµÐ°Ð»Ð¸Ð·Ð°ÑÐ¸Ð¸',bp.x,bp.y+24)}
      animRef.current=requestAnimationFrame(draw)
    }
    draw()
    const onS=e=>{drag.current=true;const p=e.touches?e.touches[0]:e;last.current={x:p.clientX,y:p.clientY}}
    const onM=e1=>{if(!drag.current)return;const p=e.touches?e.touches[0]:e;angle.current.y+=(p.clientX-last.current.x)*0.01;angle.current.x+=(p.clientY-last.current.y)*0.01;last.current={x:p.clientX,y:p.clientY}}
    const onE=()=>{drag.current=false}
    const onClick=e=>{if(onTapFace){const rect=cv.getBoundingClientRect();const cx=(e.clientX-rect.left)/rect.width*W;const cy=(e.clientY-rect.top)/rect.height*H;const ml1=[{vi:1,n:'genetic'},{vi:2,n:'emotional'},{vi:3,n:'language'},{vi:4,n:'motivation'}];for(!nof ml1){const p=proj(tf[m.vi].x,tf[m.vi].y,tf[m.vi].z);if(Math.hypot(cx-p.x,cy-p.y)<30){onTapFace(m.n);return}}}}
    ...
    return()=>{cancelAnimationFrame(animRef.current);cv.removeEventListener('mousedown',onS);cv.removeEventListener('mousemove',nM);cv.removeEventListener('mouseup',onE);cv.removeEventListener('touchstart',onS);cv.removeEventListener('touchmove',onM);cv.removeEventListener('touchend',onE);if(onTapFace)cv.removeEventListener('click',onClick)}
  },[autoSpin,sjize,height,showLabels,onTapFace])
